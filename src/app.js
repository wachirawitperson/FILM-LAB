...
/* ==================================================
   FILM LAB — DISCOVERY + SMART AUTO LAYER
   Point 1: Dynamic image aspect preview
   Point 2: Full Light Table upload/drop zone
   Point 3: Visual Film Discovery
   Point 4: Auto / Surprise Me / Manual
   ================================================== */
(() => {
  ...
  style.textContent = \`
    ...
    .filmlab-dynamic-aspect > img,
    .filmlab-dynamic-aspect > .graded-layer,
    .filmlab-dynamic-aspect > .graded-layer > canvas {
      width: 100%;
      height: 100%;
      object-fit: contain;
    }

    /* P2 — Light Table is a real upload target */
    #empty-state.filmlab-upload-target {
      cursor: pointer;
      outline: none;
      transition: border-color .18s ease, background .18s ease, transform .18s ease;
    }
    #empty-state.filmlab-upload-target:hover,
    #empty-state.filmlab-upload-target:focus-visible {
      border-color: var(--accent, #d97706);
      background: color-mix(in srgb, var(--paper-deep, #ede7db) 88%, var(--accent, #d97706));
    }
    #empty-state.filmlab-upload-target.is-dragging {
      border-color: var(--accent, #d97706);
      background: color-mix(in srgb, var(--paper-deep, #ede7db) 78%, var(--accent, #d97706));
      transform: scale(.995);
    }

    /* P3/P4 — discovery mode rail */
    .filmlab-mode-rail {
      display: flex;
      align-items: stretch;
      gap: 6px;
    ...
    .filmlab-mode-btn:hover { color: var(--ink); }
    .filmlab-mode-btn.is-active {
      background: var(--ink);
      color: var(--paper);
    }

    .filmlab-discovery-helper {
      margin: 0 0 12px;
      padding: 10px 12px;
    ...
    .filmlab-discovery-helper strong { color: var(--ink); }

    .filmlab-visual-chips {
      display: flex;
      flex-wrap: wrap;
      gap: 6px;
      margin: 0 0 10px;
    ...
    .filmlab-visual-chip:hover,
    .filmlab-visual-chip.is-active {
      color: var(--ink);
    ...
    .filmlab-auto-panel[hidden] { display: none !important; }
    .filmlab-auto-head {
      display: flex;
    ...
    .filmlab-intent.is-active {
      border-color: var(--accent, #d97706);
      color: var(--ink);
      background: color-mix(in srgb, var(--paper) 88%, var(--accent, #d97706));
    }
    .filmlab-recommendations {
      display: grid;
      grid-template-columns: repeat(3, minmax(0, 1fr));
      gap: 8px;
    }
    .filmlab-recommendation {
      min-width: 0;
      border: 1px solid var(--line);
      background: var(--paper);
      border-radius: 7px;
    ...
    .filmlab-recommendation:hover,
    .filmlab-recommendation:focus-visible {
      border-color: var(--ink);
      outline: none;
    }
    .filmlab-rec-name {
    ...
    @media (max-width: 560px) {
      .filmlab-mode-rail { gap: 3px; }
      .filmlab-mode-btn { font-size: .66rem; padding-inline: 4px; }
  ...
  const intents = [
    ["auto", "✨ Surprise me with the best match"],
    ["food", "🍜 น่ากิน / สีอาหารเด่น"],
    ["natural", "🌿 ธรรมชาติ"],
    ["film", "🎞 ฟีลฟิล์มคลาสสิก"],
    ["soft", "☁ นุ่มละมุน"],
    ["cinematic", "🎬 Cinematic"]
  ];

  function ensureDiscoveryModes() {
    const discovery = document.getElementById("discovery-section");
    if (!discovery || document.getElementById("filmlab-mode-rail")) return;

    const rail = document.createElement("div");
    rail.className = "filmlab-mode-rail";
    rail.id = "filmlab-mode-rail";
    rail.setAttribute("role", "tablist");
    rail.setAttribute("aria-label", "Film selection mode");
    ...
    [
      ...
      const b = document.createElement("button");
      b.type = "button";
      b.className = "filmlab-mode-btn";
      b.dataset.mode = mode;
      b.textContent = label;
      b.setAttribute("role", "tab");
      b.onclick = () => setSmartMode(mode);
      rail.appendChild(b);
    });

    const helper = document.createElement("div");
    helper.className = "filmlab-discovery-helper";
    helper.id = "filmlab-discovery-helper";
    helper.innerHTML = "<strong>Choose how you want to find your look.</strong> You can let FILM LAB recommend, explore visually, or control every choice yourself.";

    const visual = document.createElement("div");
    visual.className = "filmlab-visual-chips";
    visual.id = "filmlab-visual-chips";
    visual.setAttribute("aria-label", "Visual film moods");
    visualFilters.forEach(([id, label]) => {
      const b = document.createElement("button");
      b.type = "button";
      b.className = "filmlab-visual-chip";
      ...
      b.onclick = () => {
        ...
        renderGrid();
        renderRecentRow();
        visual.querySelectorAll(".filmlab-visual-chip").forEach(x => x.classList.toggle("is-active", x === b));
      };
    ...
    auto.innerHTML = \`
      <div class="filmlab-auto-head">
        <div>
          ...
          <h3 class="filmlab-auto-title">Let FILM LAB choose from your existing Looks</h3>
          <p class="filmlab-auto-note">วิเคราะห์ลักษณะภาพในเครื่องก่อน แล้วจับคู่กับ Film Looks ที่มีอยู่แล้ว — รูปไม่ถูกอัปโหลดออกจากเบราว์เซอร์</p>
        </div>
        <button class="filmlab-auto-close" type="button" aria-label="Close Auto recommendations">×</button>
      </div>
      <div class="filmlab-analysis" id="filmlab-analysis"></div>
    ...
    rail.insertAdjacentElement("afterend", helper);
    helper.insertAdjacentElement("afterend", visual);
    visual.insertAdjacentElement("afterend", auto);

    auto.querySelector(".filmlab-auto-close").onclick = () => {
      setSmartMode("manual");
    };
    ...
    intents.forEach(([id, label]) => {
      ...
      auto.querySelector("#filmlab-intents").appendChild(b);
    });
  ...
  function setSmartMode(mode) {
    smartMode = mode;
    ensureDiscoveryModes();
    updateSmartModeUI();

    if (mode === "auto") {
      const panel = document.getElementById("filmlab-auto-panel");
    ...
    if (mode === "surprise") {
      const pool = getSmartSurprisePool();
      if (pool.length) {
        const candidates = pool.slice().sort(() => Math.random() - 0.5).slice(0, 3);
        const chosen = candidates.find(p => p.id !== state.lastSurpriseId) || candidates[0];
        if (chosen) {
          state.lastSurpriseId = chosen.id;
          selectPreset(chosen);
        }
    ...
    const helper = document.getElementById("filmlab-discovery-helper");
    if (helper) helper.innerHTML = "<strong>Manual mode.</strong> Browse by visual mood, search by film name, or open the full film index.";
  }

  function updateSmartModeUI() {
    document.querySelectorAll(".filmlab-mode-btn").forEach(b => {
      const active = b.dataset.mode === smartMode;
    ...
    if (!helper) return;
    if (smartMode === "auto") {
      helper.innerHTML = "<strong>Auto mode.</strong> FILM LAB analyzes the image locally, asks one small preference, then recommends three existing Looks.";
    } else if (smartMode === "surprise") {
      helper.innerHTML = "<strong>Surprise Me.</strong> Try a few Looks without needing to know film names. The selection respects the current visual mood when possible.";
    } else {
      helper.innerHTML = "<strong>Manual mode.</strong> Choose a mood first, then browse, search, favorite, and fine-tune exactly what you want.";
    }
  }

  function getSmartSurprisePool() {
    let pool = typeof getAllPresets === "function" ? getAllPresets().filter(p => p.source !== "custom") : [];
    if (state.selectedCategory && state.selectedCategory !== "ALL") {
      const filtered = getFilteredPresets().filter(p => p.source !== "custom");
      if (filtered.length >= 3) pool = filtered;
    }
    if (!pool.length) pool = presetLibrary.slice();
    return pool;
  }

  function sampleImageAnalysis(img) {
    const max = 72;
    ...
    let lumSum = 0, satSum = 0, warmSum = 0, green = 0, skin = 0, high = 0, low = 0, n = 0;
    const hueBuckets = { red: 0, orange: 0, yellow: 0, green: 0, blue: 0, neutral: 0 };
    ...
    for (let i = 0; i < d.length; i += 4) {
      ...
      if (lum > .78) high++;
      if (lum < .22) low++;
      if (g > r * .95 && g > b * 1.08 && g > 70) green++;
      if (r > 95 && r > g * 1.08 && g > b * 1.15 && g < 205) skin++;

      let bucket = "neutral";
      if (sat > .18) {
        if (r > g * 1.28 && r > b * 1.3) bucket = "red";
        else if (r > g * 1.08 && g > b * 1.18) bucket = "orange";
        else if (r > b * 1.25 && g > b * 1.15) bucket = "yellow";
        else if (g > r * 1.05 && g > b * 1.08) bucket = "green";
        else if (b > r * 1.15 && b > g * 1.08) bucket = "blue";
      }
      hueBuckets[bucket]++;
      n++;
    }
    ...
    const warmth = warmSum / n;
    const contrast = (high + low) / n;
    const greenRatio = green / n;
    const skinRatio = skin / n;
    const dominant = Object.entries(hueBuckets).sort((a,b) => b[1]-a[1])[0][0];

    let scene = "Everyday";
    if (brightness < .28) scene = "Night / Low light";
    else if (skinRatio > .08) scene = "People / Portrait";
    else if (greenRatio > .18) scene = "Nature / Outdoors";
    else if (dominant === "orange" || dominant === "red") scene = "Food / Warm color";
    else if (contrast > .34 && saturation > .32) scene = "Colorful / High energy";

    return {
      brightness, saturation, warmth, contrast, greenRatio, skinRatio, dominant, scene
    };
  }

  function presetText(p) {
    return [
      p.name, p.description, p.character, p.category, p.collection,
      p.manufacturer, p.stock, p.group,
      ...(p.recommendedFor || []),
      ...(lookMoodTags[p.id] || [])
    ].filter(Boolean).join(" ").toLowerCase();
  }

  function scorePreset(p, a, intent) {
    const text = presetText(p);
    let score = 0;
    const rec = new Set((p.recommendedFor || []).map(x => String(x).toLowerCase()));
    const tags = new Set((lookMoodTags[p.id] || []).map(x => String(x).toLowerCase()));

    const hit = (words, amount) => {
      for (const w of words) if (text.includes(w)) score += amount;
    };
    const recHit = (words, amount) => {
      for (const w of words) if (rec.has(w) || tags.has(w)) score += amount;
    };

    if (a.scene.startsWith("Food")) {
      recHit(["daylight","everyday","golden-hour","lifestyle","summer"], 3);
      hit(["warm","color","pop","vivid","golden","summer","classic"], 1.5);
    } else if (a.scene.startsWith("People")) {
      recHit(["portrait","people","golden-hour","lifestyle","studio"], 4);
      hit(["portrait","soft","cream","warm","skin"], 1.5);
    } else if (a.scene.startsWith("Nature")) {
      recHit(["nature","landscape","outdoor","daylight","summer"], 4);
      hit(["green","fresh","pastel","summer","natural"], 1.5);
    } else if (a.scene.startsWith("Night")) {
      recHit(["night","city","street","neon","flash","indoor"], 4);
      hit(["night","flash","tungsten","blue","cinema"], 1.5);
    } else {
      recHit(["everyday","street","daylight","travel"], 3);
    }

    if (a.warmth > .07) hit(["warm","golden","cream","retro","summer"], 1.2);
    if (a.warmth < -.07) hit(["cool","blue","cinema","japanese","quiet"], 1.2);
    if (a.saturation > .34) hit(["vivid","pop","color","party","saturated"], 1.2);
    if (a.saturation < .15) hit(["soft","muted","quiet","faded","pastel","bw"], 1.2);
    if (a.contrast > .34) hit(["contrast","flash","vivid","cinema"], 1);
    if (a.contrast < .18) hit(["soft","faded","pastel","cream","milk"], 1);

    const pWarm = (p.temperature || 0) / 100;
    if (a.warmth > .05 && pWarm > .04) score += 1.5;
    if (a.warmth < -.05 && pWarm < -.04) score += 1.5;
    if (a.saturation > .3 && (p.saturation || 0) > 8) score += 1;
    if (a.saturation < .16 && (p.saturation || 0) < 5) score += 1;

    const intentRules = {
      food: [["warm","golden","color","pop","vivid","classic","summer"], 4],
      natural: [["natural","green","fresh","quiet","daylight","landscape","pastel"], 4],
      film: [["vintage","retro","faded","grain","90s","classic","disposable"], 4],
      soft: [["soft","cream","milk","pastel","dream","quiet","portrait"], 4],
      cinematic: [["cinema","cinematic","tungsten","blue-hour","night","vision3"], 4],
      auto: [[], 0]
    };
    const rule = intentRules[intent] || intentRules.auto;
    hit(rule[0], rule[1]);

    if (p.source === "custom") score -= 1;
    return score;
  }

  function analyzeAndRecommend() {
    if (!state.sourceImage) return;
    const analysis = sampleImageAnalysis(state.sourceImage);
    if (!analysis) return;
    window.__filmlabSmartAnalysis = analysis;
    renderSmartAnalysis(analysis);
    renderSmartRecommendations();
  }

  function renderSmartAnalysis(a) {
    ...
    box.replaceChildren(...chips.map(x => {
      ...
      s.className = "filmlab-analysis-pill";
      s.textContent = x;
      return s;
    }));
  }
  ...
  function renderSmartRecommendations() {
    const box = document.getElementById("filmlab-recommendations");
    const intentBox = document.getElementById("filmlab-intents");
    const a = window.__filmlabSmartAnalysis;
    if (!box || !a) return;

    if (intentBox) {
      intentBox.querySelectorAll(".filmlab-intent").forEach(b => {
        b.classList.toggle("is-active", b.dataset.intent === smartIntent);
      });
    }

    const all = typeof getAllPresets === "function"
      ? getAllPresets().filter(p => p.source !== "custom")
      : presetLibrary.slice();

    const ranked = all
      .map(p => ({ p, score: scorePreset(p, a, smartIntent) }))
      .sort((x,y) => y.score - x.score);

    const chosen = [];
    const seenFamilies = new Set();
    for (const item of ranked) {
      const family = item.p.category || item.p.collection || "other";
      if (chosen.length < 3 && !seenFamilies.has(family)) {
        chosen.push(item);
    ...
    if (chosen.length < 3) {
      for (const item of ranked) {
        if (!chosen.some(x => x.p.id === item.p.id)) chosen.push(item);
        if (chosen.length === 3) break;
      }
    }

    box.replaceChildren(...chosen.map(({p}) => {
      const b = document.createElement("button");
      b.type = "button";
  ...
  function recommendationReason(p, a, intent) {
    const reasons = [];
    if (a.scene.startsWith("Food") && /warm|golden|color|pop|vivid|classic/i.test(presetText(p))) reasons.push("เข้ากับภาพอาหารและโทนสีอุ่น");
    else if (a.scene.startsWith("People") && /portrait|soft|cream|warm/i.test(presetText(p))) reasons.push("ช่วยให้โทนคนและไฮไลต์ดูนุ่ม");
    else if (a.scene.startsWith("Nature") && /green|fresh|pastel|landscape/i.test(presetText(p))) reasons.push("เข้ากับสีเขียวและแสงกลางแจ้ง");
    else if (a.scene.startsWith("Night") && /night|cinema|tungsten|flash|blue/i.test(presetText(p))) reasons.push("รักษาบรรยากาศแสงน้อยและคอนทราสต์");
    else reasons.push("บาลานซ์กับลักษณะสีและแสงของภาพ");

    if (intent === "film") reasons.push("เพิ่ม character แบบฟิล์ม");
    if (intent === "soft") reasons.push("โทนนุ่มและไฮไลต์ละมุน");
    if (intent === "cinematic") reasons.push("โทนภาพแบบ cinematic");
    if (intent === "food") reasons.push("ช่วยให้สีอาหารดูเด่นขึ้น");
    return reasons.join(" · ");
  }

  function syncDynamicAspect() {
    const viewport = document.getElementById("stage-viewport");
    const img = document.getElementById("image-preview");
    if (!viewport || !img || !state.sourceImage) return;
    const w = state.sourceImage.naturalWidth || img.naturalWidth || state.sourceImage.width;
  ...
  function setupLightTableUpload() {
    ...
    if (!target || target.dataset.filmlabUploadBound) return;
    target.dataset.filmlabUploadBound = "true";
    target.classList.add("filmlab-upload-target");
    target.tabIndex = 0;
    target.setAttribute("role", "button");
    target.setAttribute("aria-label", "Upload a photograph");
    ...
    ["dragenter","dragover"].forEach(type => target.addEventListener(type, e => {
      e.preventDefault();
      e.stopPropagation();
      target.classList.add("is-dragging");
    }));
    ["dragleave","drop"].forEach(type => target.addEventListener(type, e => {
      e.preventDefault();
      e.stopPropagation();
  ...
  function syncSmartVisibility() {
    const hasImage = Boolean(state.sourceImage);
    const rail = document.getElementById("filmlab-mode-rail");
    if (rail) rail.hidden = !hasImage;
    const helper = document.getElementById("filmlab-discovery-helper");
    if (helper) helper.hidden = !hasImage;
    const visual = document.getElementById("filmlab-visual-chips");
    if (visual) visual.hidden = !hasImage;
    const panel = document.getElementById("filmlab-auto-panel");
    if (panel && !hasImage) panel.hidden = true;
  }

  function bootSmartLayer() {
    ensureDiscoveryModes();
    setupLightTableUpload();
    syncSmartVisibility();

    const img = document.getElementById("image-preview");
    img?.addEventListener("load", syncDynamicAspect);
    const viewport = document.getElementById("stage-viewport");
    if (viewport) {
      const observer = new MutationObserver(() => {
        if (state.sourceImage) syncDynamicAspect();
      });
      observer.observe(viewport, { attributes: true, attributeFilter: ["hidden","style","class"] });
    }
    ...
    if (shell) {
      ...
      observer.observe(shell, { attributes: true, attributeFilter: ["class","data-active-panel"] });
    }
    ...
    // Keep the three modes available after the existing discovery UI rerenders.
    const discovery = document.getElementById("discovery-section");
    if (discovery) {
      const observer = new MutationObserver(() => {
        if (!document.getElementById("filmlab-mode-rail")) ensureDiscoveryModes();
      });
      observer.observe(discovery, { childList: true, subtree: true });
    }

    window.__filmlabSmart = {
      setMode: setSmartMode,
      analyze: analyzeAndRecommend,
      getAnalysis: () => window.__filmlabSmartAnalysis || null,
      syncAspect: syncDynamicAspect
    };
  }
